...
export default async function handler(req, res){
    if(req.method == "POST"){
        ...
        const {data: rat_ids} = await supabase.from("rats").select("id")
        ...
        const file_titles_array = JSON.parse(rat_file_titles[titles.indexOf(req.body.project)].file_titles)
        const file_content_array = JSON.parse(rat_file_content[0].file_content)
